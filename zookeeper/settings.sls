...
{%- set version      = salt['grains.get']('zookeeper_version', pillar_version) %}

{%- set version_name = 'zookeeper-' + version %}
{%- set default_url  = 'http://www.us.apache.org/dist/zookeeper/' + version_name + '/' + version_name + '.tar.gz' %}
{%- set pillar_source      = salt['pillar.get']('zookeeper:source', default_url) %}
{%- set source_url   = salt['grains.get']('zookeeper:source', pillar_source) %}
